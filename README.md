...
- Planner execution budgets live in the `PlannerQuotas` block of `appsettings.*`. Provide global defaults or per-planner/per-persona overrides for `MaxIterations`, `MaxQueuedJobs`, and `MaxTokens`. When a request exceeds a quota the API emits `planner.throttled` (soft stop) or `planner.rejected` (hard stop) telemetry and the response metadata includes the offending limit so jobs and dashboards can surface quota hits.
